...
int WashNormalSpinState = 0; int WashRapidSpinState = 0; float MotorSpinStateChangeTime = 0; int drySpinState = 0;
float WashNormalSpinTime = 6; float WashNormalStopTime = .5; 
float WashRapidSpinTime = 1; float WashRapidStopTime = .5;

float pSensorMaximum = 628;
float pSensorMinimum = 560;
...
float eepromValue16 = 0; //pSensorMaximum
...
//  EEPROM.write(15, 170);
  ...
  eepromValue15 = EEPROM.read(14); soakTimeLeft = eepromValue15*60; //Serial.print("soakTimeLeft="); Serial.print(soakTimeLeft); Serial.print("\n");
    ...
    {
    Serial.print("String begin"); Serial.print("|"); 
...
Serial.print("Starting washing machine|");
...
void loop() {
  ...
  if (serialPrintTimerCurrent-serialPrintTimerPrevious>=1000) {Serial.print ("\n"); serialPrints();  serialPrintTimerPrevious=serialPrintTimerCurrent;}
...
//    Serial.print("Paused"); Serial.print("\t");
    ...
    {
      swl = 96; digitalWrite(lowWaterLevelLed, LOW); digitalWrite(mediumWaterLevelLed, LOW); digitalWrite(fullWaterLevelLed, HIGH);
...
else if(data == 'l') { wlSelectorCount = 1;}  
else if(data == 'm') { wlSelectorCount = 2;} 
else if(data == 'o') { wlSelectorCount = 3;}  
else if(data == 'n') { wlSelectorCount = 4;} 
  ...
  if (play == 1)
    {
      ...
      if (mSelectorCount == 2)
      {Serial.print("Selected water level="); Serial.print("|"); Serial.print("5"); Serial.print("|");
      }
      if (mSelectorCount != 2)
      {
      Serial.print("Selected water level="); Serial.print("|"); Serial.print(swl); Serial.print("|");
      }
...
//Timer
        ...
        //pressureSensorRead
//            Serial.print("CPSV="); Serial.print(currentSensorValue); Serial.print("\t=");
//            Serial.print("PSSAV="); Serial.print(smoothedSensorValueAvg); Serial.print("\t=");
//            Serial.print("AWL="); Serial.print(actualWaterLevel); Serial.print("\t");
...
void eepromWrite() {
  ...
  if (modeAccomplished == 0)
  {
    ...
    EEPROM.write(15, pSensorMaximum / 4);
...
//  Serial.print("pSMin="); Serial.print(pSensorMinimum); Serial.print("\t=");
//  Serial.print("pSMax="); Serial.print(pSensorMaximum); Serial.print("\t=");
//  Serial.print("raw sensor value="); Serial.print(currentSensorValue); Serial.print("\t=");
//  Serial.print("PSSAV="); Serial.print(smoothedSensorValueAvg); Serial.print("\t=");
//  Serial.print("AWL="); Serial.print(actualWaterLevel); Serial.print("\t");
}
...
//  Serial.print("MotorSpinStateChangeTime = "); Serial.print(MotorSpinStateChangeTime); Serial.print("\t"); 
    ...
    if (WashRapidSpinState == 0) //stopped motor
  {
    stopMotor(); //Serial.print("waitForSoakTime = "); Serial.print(waitForSoakTime); Serial.print("\t"); 
    //Serial.print ("Change time= "); //Serial.print(MotorSpinStateChangeTime);  //Serial.print ("\t"); //Serial.print ("Current time= "); //Serial.print( timer); //Serial.print ("\t"); //Serial.print ("Motor stopped"); //Serial.print ("\t");
    if ( soakTimeLeft <= 0) {
  ...
  if (WashRapidSpinState == 1) //switching cw
  {
    stopMotor();
    cw();
    //Serial.print ("Change time= "); //Serial.print(MotorSpinStateChangeTime);  //Serial.print ("\t"); //Serial.print ("Current time= "); //Serial.print( timer); //Serial.print ("\t"); //Serial.print ("Motor stopped"); //Serial.print ("\t");
  ...
  if (WashRapidSpinState == 2) //running
  {
    startMotor();
    //Serial.print ("Change time= "); //Serial.print(MotorSpinStateChangeTime);  //Serial.print ("\t"); //Serial.print ("Current time= "); //Serial.print( timer); //Serial.print ("\t");   //Serial.print ("Motor spinning cw"); //Serial.print ("\t");
  ...
  if (WashRapidSpinState == 3) //stopped
  {
    stopMotor();
    //Serial.print ("Change time= "); //Serial.print(MotorSpinStateChangeTime);  //Serial.print ("\t"); //Serial.print ("Current time= "); //Serial.print( timer); //Serial.print ("\t"); //Serial.print ("Motor stopped"); //Serial.print ("\t");
    if ( timer - MotorSpinStateChangeTime > WashRapidStopTime) {
      MotorSpinStateChangeTime =  timer; 
      WashRapidSpinState = 4;
    }
  }

  if (WashRapidSpinState == 4) //switching ccw
  {
    stopMotor(); 
    ccw();
    //Serial.print ("Change time= "); //Serial.print(MotorSpinStateChangeTime);  //Serial.print ("\t"); //Serial.print ("Current time= "); //Serial.print( timer); //Serial.print ("\t"); //Serial.print ("Motor stopped"); //Serial.print ("\t");
    if ( timer - MotorSpinStateChangeTime > .5) {
      MotorSpinStateChangeTime =  timer; 
      WashRapidSpinState = 5;
    }
  }
  
  if (WashRapidSpinState == 5) //running
  {
    startMotor();
    //Serial.print ("Change time= "); //Serial.print(MotorSpinStateChangeTime);  //Serial.print ("\t"); //Serial.print ("Current time= "); //Serial.print( timer); //Serial.print ("\t");   //Serial.print ("Motor spinning cw"); //Serial.print ("\t");
    if ( timer - MotorSpinStateChangeTime > WashRapidSpinTime) {
      MotorSpinStateChangeTime =  timer;
      WashRapidSpinState = 6;
    }
  }

    if (WashRapidSpinState == 6) //stopped
  {
    stopMotor();
    //Serial.print ("Change time= "); //Serial.print(MotorSpinStateChangeTime);  //Serial.print ("\t"); //Serial.print ("Current time= "); //Serial.print( timer); //Serial.print ("\t"); //Serial.print ("Motor stopped"); //Serial.print ("\t");
    if ( timer - MotorSpinStateChangeTime > WashRapidStopTime) {
      ...
      if (spinRequest == 0)
      {
        ...
        if (ignoreWaterLevel == 1)
        {
          ...
          if (washModeStarted == 1) {
            ...
            if (washModeTimeLeft > 0)
            {
             if (washModeTimeLeft >= (.66*WashCycleTime))
              {
                //lcd.setCursor(0, 1);
                //lcd.print("Washing      ");
                motorNormalSpin();//start wash cycle
             ...
             if ((washModeTimeLeft < (.66*WashCycleTime))&&(washModeTimeLeft >= (.5*WashCycleTime)))
              {
                //lcd.setCursor(0, 1);
                //lcd.print("Washing      ");
                motorRapidSpin();//start wash cycle
              }
              
             if ((washModeTimeLeft < (.5*WashCycleTime))&&(washModeTimeLeft >= (.16*WashCycleTime)))
              {
                //lcd.setCursor(0, 1);
                //lcd.print("Washing      ");
                motorNormalSpin();//start wash cycle
             ...
             if (washModeTimeLeft < (.16*WashCycleTime))
              {
                //lcd.setCursor(0, 1);
                //lcd.print("Washing      ");
                motorRapidSpin();//start wash cycle
